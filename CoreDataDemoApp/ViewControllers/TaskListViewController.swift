...
extension TaskListViewController{
    ...
    private func showAlert(task: Task? = nil, completion: (() -> Void)? = nil) {
        let title = task != nil ? "Update" : "New Task"
        let alert = UIAlertController(title: title, message: "What you want to do?", preferredStyle: .alert)
        
        alert.action(task: task) { taskName in
            if let task = task, let completion = completion {
                StorageManager.shared.editTask(task: task, taskName: taskName)
                completion()
            } else {
                self.save(taskName)
            }
